...
def main():
	...
	try:
		...
		if pilih == '1':
			...
			print
			os.system('xdg-open https://www.ryuxploit.xyz')
			sleep(2)
			print(W+'Abis Report Apa'+C+' ^_^')
			sys.exit()
			
		elif pilih == '5':
			print
			os.system('xdg-open https://m.youtube.com/channel/UC0j4M2aL0U1Ay9rPWstxzTA')
			sleep(2)
			tanya = raw_input(W+'Udah Subscribe Belum ?'+C+' ^_^'+W+' ['+C+'Y'+W+'/'+C+'n'+W+']'+C+' : '+W+'')
			if tanya.upper() == 'done':
				print
				print(W+'Thank u very much'+C+' ^_^')
				sys.exit() 
				
			else:
				print
				print(W+'Ok thanks'+C+' ^_^')
				sys.exit() 
